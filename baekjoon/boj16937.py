...
for cur in johab:
    ...
    if rest_r>=0 and rest_c>=0:
        ...
        if (r2<=rest_r and c2<=w) or (c2<=rest_r and r2<=w):
            answer = max(answer, r1*c1+r2*c2)
        elif  (r2<=h and c2<=rest_c) or (c2<=h and r2<=rest_c):
            answer = max(answer, r1*c1+r2*c2)
    
    if rest_r2>=0 and rest_c2>=0:
        if (r2<=rest_r2 and c2<=w) or (c2<=rest_r2 and r2<=w):
            answer = max(answer, r1*c1+r2*c2)
        elif (r2<=h and c2<=rest_c2) or (c2<=h and r2<=rest_c2):
            answer = max(answer, r1*c1+r2*c2)
